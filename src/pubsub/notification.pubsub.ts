...
export class NotificationPubSub {
  ...
  public dispatch = async (user: User, messageContent: string): Promise<void> => {
    ...
    for (const [preference, enabled] of Object.entries(user.preferences)) {
      ...
      taskGroups[config.endpoint].tasks.push(this.createNotificationTask(config, data));
    }
  ...
  private createNotificationTask = (config: NotificationConfigItem, data: any): () => Promise<void> => {
    return async () => {
      try {
        await axios({
